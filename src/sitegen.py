...
def generateSite(cards: List[Card], diff: List[Card]):
...
def writeAllCards(cards:List[Card], diff:List[Card], outfile: TextIOWrapper):
	for status in [-1, 0, 1, 2]:
		section = [card for card in cards if card.status == status]
		writeSection(section, diff, outfile)
	
	unlim = [card for card in cards if card.status == 3 and card in diff]
	if len(unlim) > 0:
		writeSection(unlim, diff, outfile)


def writeSection(cards:List[Card], diff:List[Card], outfile:TextIOWrapper):
...
def writeSubSection(cards:List[Card], diff:List[Card], outfile:TextIOWrapper):
	for card in sorted(cards, key=lambda x: x.name.lower()):
		writeCard(card, diff, outfile)

def writeCard(card:Card, diff:List[Card], outfile:TextIOWrapper):
